...
(def DatasetField-schema
  [:enum
   {:closed true,
    ...
    :gcp/key :gcp.bigquery/BigQuery.DatasetField} "ACCESS" "CREATION_TIME"
   "DATASET_REFERENCE" "DEFAULT_TABLE_EXPIRATION_MS" "DESCRIPTION" "ETAG"
   "FRIENDLY_NAME" "ID" "LABELS" "LAST_MODIFIED_TIME" "LOCATION" "SELF_LINK"])
...
(def DatasetView-schema
  [:enum
   {:closed true,
    :doc nil,
    :gcp/category :nested/enum,
    :gcp/key :gcp.bigquery/BigQuery.DatasetView} "DATASET_VIEW_UNSPECIFIED"
   "FULL" "METADATA" "ACL"])
...
(def DatasetUpdateMode-schema
  [:enum
   {:closed true,
    :doc nil,
    :gcp/category :nested/enum,
    :gcp/key :gcp.bigquery/BigQuery.DatasetUpdateMode} "UPDATE_MODE_UNSPECIFIED"
   "UPDATE_FULL" "UPDATE_METADATA" "UPDATE_ACL"])
...
(def TableField-schema
  [:enum
   {:closed true,
    ...
    :gcp/category :nested/enum,
    :gcp/key :gcp.bigquery/BigQuery.TableField} "CREATION_TIME" "DESCRIPTION"
   "ETAG" "EXPIRATION_TIME" "EXTERNAL_DATA_CONFIGURATION" "FRIENDLY_NAME" "ID"
   "LABELS" "LAST_MODIFIED_TIME" "LOCATION" "NUM_BYTES" "NUM_LONG_TERM_BYTES"
   "NUM_ROWS" "SCHEMA" "SELF_LINK" "STREAMING_BUFFER" "TABLE_REFERENCE"
   "TIME_PARTITIONING" "RANGE_PARTITIONING" "TYPE" "VIEW"])
...
(def TableMetadataView-schema
  [:enum
   {:closed true,
    ...
    :gcp/category :nested/enum,
    :gcp/key :gcp.bigquery/BigQuery.TableMetadataView} "BASIC" "FULL"
...
(def ModelField-schema
  [:enum
   {:closed true,
    ...
    :gcp/category :nested/enum,
    :gcp/key :gcp.bigquery/BigQuery.ModelField} "CREATION_TIME" "DESCRIPTION"
   "ETAG" "EXPIRATION_TIME" "FRIENDLY_NAME" "LABELS" "LAST_MODIFIED_TIME"
...
(def RoutineField-schema
  [:enum
   {:closed true,
    ...
    :gcp/category :nested/enum,
    :gcp/key :gcp.bigquery/BigQuery.RoutineField} "ARGUMENTS" "CREATION_TIME"
   "DEFINITION_BODY" "ETAG" "IMPORTED_LIBRARIES" "LANGUAGE" "LAST_MODIFIED_TIME"
   "RETURN_TYPE" "ROUTINE_REFERENCE" "ROUTINE_TYPE"])
...
(def JobField-schema
  [:enum
   {:closed true,
    ...
    :gcp/category :nested/enum,
    :gcp/key :gcp.bigquery/BigQuery.JobField} "CONFIGURATION" "ETAG" "ID"
   "JOB_REFERENCE" "SELF_LINK" "STATISTICS" "STATUS" "USER_EMAIL"])
        ...
        (reduce-kv
          (fn [acc k v]
            (case k
              ...
              :pageSize (conj acc
                              (BigQuery$DatasetListOption/pageSize (long v)))
      ...
      (reduce-kv
        (fn [acc k v]
          (case k
            ...
            :pageSize (reduced (BigQuery$DatasetListOption/pageSize (long v)))
...
(def DatasetListOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/key :gcp.bigquery/BigQuery.DatasetListOption}
   [:map {:closed true}
    [:labelFilter
     {:optional true,
      :doc
...
(do
  (defn ^BigQuery$DatasetOption/1 DatasetOption-Array-from-edn
    [arg]
    (global/strict! :gcp.bigquery/BigQuery.DatasetOption arg)
    (into-array
...
(def DatasetOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.DatasetOption}
   [:map {:closed true}
    [:fields
     {:optional true,
      :doc
...
(def DatasetDeleteOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.DatasetDeleteOption}
   [:map {:closed true}
    [:deleteContents
     {:optional true,
      :doc
...
(do (defn ^BigQuery$ModelListOption/1 ModelListOption-Array-from-edn
      [arg]
      (global/strict! :gcp.bigquery/BigQuery.ModelListOption arg)
      (into-array
        BigQuery$ModelListOption
        (reduce-kv
          (fn [acc k v]
            (case k
              :pageSize (conj acc (BigQuery$ModelListOption/pageSize (long v)))
    ...
    (defn ^BigQuery$ModelListOption ModelListOption-from-edn
      ...
      (reduce-kv (fn [acc k v]
                   (case k
                     :pageSize (reduced (BigQuery$ModelListOption/pageSize
                                          (long v)))
...
(def ModelListOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.ModelListOption}
   [:map {:closed true}
    [:pageSize
     {:optional true,
      :doc
...
(do (defn ^BigQuery$RoutineListOption/1 RoutineListOption-Array-from-edn
      [arg]
      (global/strict! :gcp.bigquery/BigQuery.RoutineListOption arg)
      (into-array
        ...
        (reduce-kv
          (fn [acc k v]
            (case k
              :pageSize (conj acc
                              (BigQuery$RoutineListOption/pageSize (long v)))
    ...
    (defn ^BigQuery$RoutineListOption RoutineListOption-from-edn
      ...
      (reduce-kv
        (fn [acc k v]
          (case k
            :pageSize (reduced (BigQuery$RoutineListOption/pageSize (long v)))
            :pageToken (reduced (BigQuery$RoutineListOption/pageToken v))
            acc))
        nil
        arg)))
...
(def RoutineListOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.RoutineListOption}
   [:map {:closed true}
    [:pageSize
     {:optional true,
      :doc
...
(do (defn ^BigQuery$TableListOption/1 TableListOption-Array-from-edn
      [arg]
      (global/strict! :gcp.bigquery/BigQuery.TableListOption arg)
      (into-array
        BigQuery$TableListOption
        (reduce-kv
          (fn [acc k v]
            (case k
              :pageSize (conj acc (BigQuery$TableListOption/pageSize (long v)))
    ...
    (defn ^BigQuery$TableListOption TableListOption-from-edn
      ...
      (reduce-kv (fn [acc k v]
                   (case k
                     :pageSize (reduced (BigQuery$TableListOption/pageSize
                                          (long v)))
...
(def TableListOption-schema
  [:maybe
   {:closed true,
    :doc "Class for specifying table list options.",
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.TableListOption}
   [:map {:closed true}
    [:pageSize
     {:optional true,
      :doc
...
(def TableOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.TableOption}
   [:map {:closed true}
    [:fields
     {:optional true,
      :doc
...
(do (defn ^BigQuery$IAMOption/1 IAMOption-Array-from-edn
      [arg]
      (global/strict! :gcp.bigquery/BigQuery.IAMOption arg)
      (into-array
        BigQuery$IAMOption
        (reduce-kv
          (fn [acc k v]
            (case k
              :requestedPolicyVersion
                (conj acc (BigQuery$IAMOption/requestedPolicyVersion (long v)))
              acc))
          []
          arg)))
    (defn ^BigQuery$IAMOption IAMOption-from-edn
      ...
      (reduce-kv (fn [acc k v]
                   (case k
                     :requestedPolicyVersion
                       (reduced (BigQuery$IAMOption/requestedPolicyVersion
                                  (long v)))
                     acc))
                 nil
                 arg)))
...
(def IAMOption-schema
  [:maybe
   {:closed true,
    :doc nil,
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.IAMOption}
   [:map {:closed true} [:requestedPolicyVersion {:optional true} :i64]]])
...
(def ModelOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.ModelOption}
   [:map {:closed true}
    [:fields
     {:optional true,
      :doc
...
(def RoutineOption-schema
  [:maybe
   {:closed true,
    :doc "Class for specifying table get, create and update options.",
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.RoutineOption}
   [:map {:closed true}
    [:fields
     {:optional true,
      :doc
...
(do (defn
      ...
      [arg]
      (global/strict! :gcp.bigquery/BigQuery.TableDataListOption arg)
      (into-array
        ...
        (reduce-kv
          (fn [acc k v]
            (case k
              :pageSize (conj acc
                              (BigQuery$TableDataListOption/pageSize (long v)))
              :pageToken (conj acc (BigQuery$TableDataListOption/pageToken v))
              :startIndex
                (conj acc (BigQuery$TableDataListOption/startIndex (long v)))
              acc))
          []
          arg)))
    (defn ^BigQuery$TableDataListOption TableDataListOption-from-edn
      ...
      (reduce-kv
        (fn [acc k v]
          (case k
            :pageSize (reduced (BigQuery$TableDataListOption/pageSize (long v)))
            :pageToken (reduced (BigQuery$TableDataListOption/pageToken v))
            :startIndex (reduced (BigQuery$TableDataListOption/startIndex (long
                                                                            v)))
            acc))
        nil
        arg)))
...
(def TableDataListOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.TableDataListOption}
   [:map {:closed true}
    [:pageSize
     {:optional true,
      :doc
      ...
      (reduce-kv
        (fn [acc k v]
          (case k
            ...
            :maxCreationTime
              (conj acc (BigQuery$JobListOption/maxCreationTime (long v)))
            :pageSize (conj acc (BigQuery$JobListOption/pageSize (long v)))
      ...
      (fn [acc k v]
        (case k
          ...
          :maxCreationTime (reduced (BigQuery$JobListOption/maxCreationTime
                                      (long v)))
          :pageSize (reduced (BigQuery$JobListOption/pageSize (long v)))
...
(def JobListOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.JobListOption}
   [:map {:closed true}
    [:allUsers
     {:optional true,
      :doc
...
(def JobOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.JobOption}
   [:map {:closed true}
    [:fields
     {:optional true,
      :doc
    ...
    [:bigQueryRetryConfig
     ...
     :gcp.bigquery/BigQueryRetryConfig]
    [:retryOptions
     {:optional true,
...
(do
  (defn ^BigQuery$QueryResultsOption/1 QueryResultsOption-Array-from-edn
    [arg]
    (global/strict! :gcp.bigquery/BigQuery.QueryResultsOption arg)
    (into-array
      BigQuery$QueryResultsOption
      (reduce-kv
        (fn [acc k v]
          (case k
            :pageSize (conj acc (BigQuery$QueryResultsOption/pageSize (long v)))
            :pageToken (conj acc (BigQuery$QueryResultsOption/pageToken v))
            :startIndex (conj acc
                              (BigQuery$QueryResultsOption/startIndex (long v)))
            :maxWaitTime
              (conj acc (BigQuery$QueryResultsOption/maxWaitTime (long v)))
            acc))
        []
        arg)))
  (defn ^BigQuery$QueryResultsOption QueryResultsOption-from-edn
    ...
    (reduce-kv
      (fn [acc k v]
        (case k
          :pageSize (reduced (BigQuery$QueryResultsOption/pageSize (long v)))
          :pageToken (reduced (BigQuery$QueryResultsOption/pageToken v))
          :startIndex (reduced (BigQuery$QueryResultsOption/startIndex (long
                                                                         v)))
          :maxWaitTime (reduced (BigQuery$QueryResultsOption/maxWaitTime (long
                                                                           v)))
          acc))
      nil
      arg)))
...
(def QueryResultsOption-schema
  [:maybe
   {:closed true,
    ...
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.QueryResultsOption}
   [:map {:closed true}
    [:pageSize
     {:optional true,
      :doc
...
(do (defn ^BigQuery$QueryOption/1 QueryOption-Array-from-edn
      [arg]
      (global/strict! :gcp.bigquery/BigQuery.QueryOption arg)
...
(def QueryOption-schema
  [:maybe
   {:closed true,
    :doc nil,
    :gcp/category :nested/client-options,
    :gcp/key :gcp.bigquery/BigQuery.QueryOption}
   [:map {:closed true}
