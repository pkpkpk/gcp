...
(ns gcp.bindings.bigquery.DatasetInfo
  {:doc
   ...
   :gcp.dev/certification
     {:base-seed 1772045815933
      :manifest "32051bbb-16c0-5b08-9f8c-751cde0c9bfb"
      :passed-stages
        {:smoke 1772045815933 :standard 1772045815934 :stress 1772045815935}
      :protocol-hash
        "b8a5eb17212acb18f49dcb7f15a243eac9b32f54c0f054a99be660a3a25e0315"
      :timestamp "2026-02-25T18:56:56.953198716Z"}}
  (:require [gcp.bindings.bigquery.Acl :as Acl]
            [gcp.bindings.bigquery.DatasetId :as DatasetId]
            [gcp.bindings.bigquery.EncryptionConfiguration :as
             EncryptionConfiguration]
            [gcp.bindings.bigquery.ExternalDatasetReference :as
...
(defn ^DatasetInfo from-edn
  [arg]
  (global/strict! :gcp.bindings.bigquery/DatasetInfo arg)
  (let [builder (DatasetInfo/newBuilder (DatasetId/from-edn (get arg
                                                                 :datasetId)))]
    (when (some? (get arg :acl))
    ...
    (when (some? (get arg :defaultPartitionExpirationMs))
      (.setDefaultPartitionExpirationMs builder
                                        (get arg
                                             :defaultPartitionExpirationMs)))
    (when (some? (get arg :defaultTableLifetime))
      (.setDefaultTableLifetime builder (get arg :defaultTableLifetime)))
    ...
    (when (some? (get arg :maxTimeTravelHours))
      (.setMaxTimeTravelHours builder (get arg :maxTimeTravelHours)))
...
(defn to-edn
  [^DatasetInfo arg]
  {:post [(global/strict! :gcp.bindings.bigquery/DatasetInfo %)]}
  (cond-> {:datasetId (DatasetId/to-edn (.getDatasetId arg))}
    (.getAcl arg) (assoc :acl (map Acl/to-edn (.getAcl arg)))
    ...
    (.getDefaultEncryptionConfiguration arg)
      (assoc :defaultEncryptionConfiguration
        (EncryptionConfiguration/to-edn (.getDefaultEncryptionConfiguration
                                          arg)))
    (.getDefaultPartitionExpirationMs arg) (assoc :defaultPartitionExpirationMs
    ...
    (.getExternalDatasetReference arg) (assoc :externalDatasetReference
                                         (ExternalDatasetReference/to-edn
    ...
    (.getStorageBillingModel arg) (assoc :storageBillingModel
                                    (.getStorageBillingModel arg))))
...
(def schema
  [:map
   {:closed true,
    ...
    :gcp/key :gcp.bindings.bigquery/DatasetInfo}
   [:acl
    {:optional true,
     :getter-doc
    ...
    [:sequential {:min 1} :gcp.bindings.bigquery/Acl]]
   [:creationTime
    {:optional true,
     :read-only? true,
     :getter-doc
   ...
   [:externalDatasetReference
    ...
    :gcp.bindings.bigquery/ExternalDatasetReference]
   [:friendlyName
    {:optional true,
...
(global/include-schema-registry!
  (with-meta {:gcp.bindings.bigquery/DatasetInfo schema}
    {:gcp.global/name "gcp.bindings.bigquery.DatasetInfo"}))