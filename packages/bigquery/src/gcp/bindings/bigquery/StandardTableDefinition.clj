...
(ns gcp.bindings.bigquery.StandardTableDefinition
  {:doc
   ...
   :gcp.dev/certification
     {:base-seed 1771340483720
      :manifest "32051bbb-16c0-5b08-9f8c-751cde0c9bfb"
      :passed-stages
        {:smoke 1771340483720 :standard 1771340483721 :stress 1771340483722}
      :protocol-hash
        "32175171656ac95c72455bff25fe019346fb52c916efee805be149692e31cec4"
      :timestamp "2026-02-17T15:01:43.081647420Z"}}
  (:require [gcp.bindings.bigquery.BigLakeConfiguration :as
...
(declare StandardTableDefinition$StreamingBuffer-from-edn
         StandardTableDefinition$StreamingBuffer-to-edn)

(defn
  ^StandardTableDefinition$StreamingBuffer StandardTableDefinition$StreamingBuffer-from-edn
  [arg]
  (throw
    (Exception.
...
(def StandardTableDefinition$StreamingBuffer-schema
  [:map
   {:closed true,
    :doc
    ...
    :gcp/key :gcp.bindings.bigquery/StandardTableDefinition.StreamingBuffer}
   [:estimatedBytes
    {:read-only? true,
     :doc
...
(defn ^StandardTableDefinition from-edn
  [arg]
  (global/strict! :gcp.bindings.bigquery/StandardTableDefinition arg)
  (let [builder (StandardTableDefinition/newBuilder)]
    ...
    (when (some? (get arg :numActiveLogicalBytes))
      (.setNumActiveLogicalBytes builder (get arg :numActiveLogicalBytes)))
    (when (some? (get arg :numActivePhysicalBytes))
      (.setNumActivePhysicalBytes builder (get arg :numActivePhysicalBytes)))
    (when (some? (get arg :numBytes))
      (.setNumBytes builder (get arg :numBytes)))
    (when (some? (get arg :numLongTermBytes))
      (.setNumLongTermBytes builder (get arg :numLongTermBytes)))
    (when (some? (get arg :numLongTermLogicalBytes))
      (.setNumLongTermLogicalBytes builder (get arg :numLongTermLogicalBytes)))
    (when (some? (get arg :numLongTermPhysicalBytes))
      (.setNumLongTermPhysicalBytes builder
                                    (get arg :numLongTermPhysicalBytes)))
    (when (some? (get arg :numRows)) (.setNumRows builder (get arg :numRows)))
    (when (some? (get arg :numTimeTravelPhysicalBytes))
      (.setNumTimeTravelPhysicalBytes builder
                                      (get arg :numTimeTravelPhysicalBytes)))
    (when (some? (get arg :numTotalLogicalBytes))
      (.setNumTotalLogicalBytes builder (get arg :numTotalLogicalBytes)))
    (when (some? (get arg :numTotalPhysicalBytes))
      (.setNumTotalPhysicalBytes builder (get arg :numTotalPhysicalBytes)))
    ...
    (when (some? (get arg :streamingBuffer))
      (.setStreamingBuffer builder
                           (StandardTableDefinition$StreamingBuffer-from-edn
                             (get arg :streamingBuffer))))
...
(defn to-edn
  ...
  {:post [(global/strict! :gcp.bindings.bigquery/StandardTableDefinition %)]}
  (cond-> {:type "TABLE"}
...
(def schema
  [:map
   {:closed true,
    ...
    :gcp/key :gcp.bindings.bigquery/StandardTableDefinition}
   [:type [:= "TABLE"]]
   [:numBytes
    {:optional true,
     :getter-doc
   ...
   [:bigLakeConfiguration
    ...
    :gcp.bindings.bigquery/BigLakeConfiguration]
...
(global/include-schema-registry!
  (with-meta {:gcp.bindings.bigquery/StandardTableDefinition schema,
              :gcp.bindings.bigquery/StandardTableDefinition.StreamingBuffer
                StandardTableDefinition$StreamingBuffer-schema}
    {:gcp.global/name "gcp.bindings.bigquery.StandardTableDefinition"}))