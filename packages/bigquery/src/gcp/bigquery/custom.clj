(ns gcp.bigquery.custom
  (:require [clojure.walk :as walk]
            [gcp.bindings.bigquery.PolicyTags :as PolicyTags]
...
(defn FieldElementType-to-edn [^FieldElementType arg]
  {:post [(g/strict! ::FieldElementType %)]}
...
(def _Field-fields
  [:map
   ...
   [:policyTags
    ...
    :gcp.bindings.bigquery/PolicyTags]
   [:precision
    {:optional true,
     :getter-doc
   ...
   [:timestampPrecision
    ...
    [:enum 6 12]]
   ;;----
   [:name {:getter-doc "Returns the field name."} [:string {:min 1}]]])
...
(def _Field-record-schema
  (conj _Field-fields
        ...
        [:subFields
         ...
         [:sequential {:min 1} [:ref :gcp.bigquery.custom/Field]]]))
...
(def Field-schema
  [:or
   {:doc "Google BigQuery Table schema field. A table field has a name, a type, a mode and possibly a description."
    ...
    :gcp/key :gcp.bigquery.custom/Field}
...
(defn ^Field Field-from-edn
  ...
  (g/strict! :gcp.bigquery.custom/Field arg)
...
(defn Field-to-edn
  [^Field arg]
  {:post [(g/strict! :gcp.bigquery.custom/Field %)]}
...
(defn ^QueryParameterValue QueryParameterValue-from-edn [arg]
  (g/strict! ::QueryParameterValue arg)
  (cond
    ...
    (map? arg)
    (cond
      (g/valid? ::Range arg)    (QueryParameterValue/range (Range-from-edn arg))
...
(defn QueryParameterValue-to-edn [^QueryParameterValue arg]
  {:post [(g/valid? ::QueryParameterValue %)]}
  (cond
...
(def InsertAllRequest$RowValue-schema
  [:or
   ...
   'bytes?
   homogenous-sequence-schemas
   [:sequential {:min 1} [:ref ::InsertAllRequest$RowToInsert]]
   [:ref ::InsertAllRequest$RowToInsert]])

(def InsertAllRequest$RowToInsert-schema
  [:map-of
   [:or :string 'simple-keyword?]
   [:ref ::InsertAllRequest$RowValue]])
...
(defn serialize-scalar
  ...
  (cond
    ...
    (cond
      ...
      (g/valid? ::Range val)     (let [{:keys [end start type]} val
                                       xf (case type
                                            "DATE" str
                                            "DATETIME" DateTime->String
                                            "TIMESTAMP" Instant->String)]
...
(def InsertAllRequest-schema
  [:map
   [:table :gcp.bindings.bigquery/TableId]
   [:rows [:sequential {:min 1} ::InsertAllRequest$RowToInsert]]
...
(g/include-schema-registry!
  (with-meta {::Field                        Field-schema
              ::FieldElementType             FieldElementType-schema
              ::Range                        Range-schema
              ::QueryParameterValue.SCALAR   scalar-schema
              ::QueryParameterValue          QueryParameterValue-schema
              ::InsertAllRequest$RowValue    InsertAllRequest$RowValue-schema
              ::InsertAllRequest$RowToInsert InsertAllRequest$RowToInsert-schema
              ::InsertAllRequest             InsertAllRequest-schema}
