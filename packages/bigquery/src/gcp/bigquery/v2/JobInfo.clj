...
(defn ^JobInfo from-edn
  ...
  (global/strict! :gcp.bigquery.v2/JobInfo arg)
...
(defn to-edn [^JobInfo arg]
  {:pre [(some? arg)]
   :post [(global/strict! :gcp.bigquery.v2/JobInfo %)]}
...
(def schemas
  {:gcp.bigquery.v2/JobInfo
   [:map
    ...
    [:configuration :gcp.bigquery.v2/JobConfiguration]
    [:statistics {:optional true} :gcp.bigquery.v2/JobStatistics]
    [:etag {:optional true} :string]
    [:generatedId {:optional true} :string]
    [:status {:optional true} :gcp.bigquery.v2/JobStatus]
