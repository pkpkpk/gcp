...
(defn ^TableInfo from-edn
  ...
  (global/strict! :gcp.bigquery.v2/TableInfo arg)
  (let [table-def (cond
...
(defn to-edn [^TableInfo arg]
  {:pre [(some? arg)]
   :post [(global/strict! :gcp.bigquery.v2/TableInfo %)]}
...
(def schemas
  {:gcp.bigquery.v2/TableInfo
   [:map
    ...
    [:definition {:optional true} :gcp.bigquery.v2/TableDefinition]
    [:cloneDefinition {:optional true} :gcp.bigquery.v2/CloneDefinition]
    ...
    [:resourceTags {:optional true} [:map-of :string :string]]
   ...
   :gcp.bigquery.v2/CloneDefinition
   [:map {:doc "read only"}
    [:baseTableId :gcp.bigquery.v2/TableId]
    [:dateTime :string]]})
