...
(def QueryJobConfiguration$Priority-schema
  [:enum
   {:closed       true,
    ...
    :gcp/key      :gcp.bigquery.custom/QueryJobConfiguration.Priority}
   "INTERACTIVE" "BATCH"])
...
(def QueryJobConfiguration$JobCreationMode-schema
  [:enum
   {:closed       true,
    ...
    :gcp/category :nested/enum,
    :gcp/key      :gcp.bigquery.custom/QueryJobConfiguration.JobCreationMode}
   "JOB_CREATION_MODE_UNSPECIFIED" "JOB_CREATION_REQUIRED"
   "JOB_CREATION_OPTIONAL"])

(defn ^QueryJobConfiguration from-edn
  [arg]
  (g/strict! :gcp.bigquery.custom/QueryJobConfiguration arg)
...
(defn to-edn
  ...
  {:post [(g/strict! :gcp.bigquery.custom/QueryJobConfiguration %)]}
...
(def fields
  [[:type [:= "QUERY"]]
   ...
   [:destinationEncryptionConfiguration {:optional true} :gcp.bindings.bigquery/EncryptionConfiguration]
   [:destinationTable
    {:optional true,
     :getter-doc
...
(def positional-schema
  (into [:map {:closed true}
         [:positionalParameters
          ...
          [:sequential :gcp.bigquery.custom/QueryParameterValue]]]
        fields))
...
(def schema
  [:or
   {:closed true,
    ...
    :gcp/key :gcp.bigquery.custom/QueryJobConfiguration}
   named-schema
   positional-schema
   no-params-schema])

(g/include-schema-registry!
  (with-meta {:gcp.bigquery.custom/QueryJobConfiguration schema,
              :gcp.bigquery.custom/QueryJobConfiguration.JobCreationMode QueryJobConfiguration$JobCreationMode-schema,
              :gcp.bigquery.custom/QueryJobConfiguration.Priority QueryJobConfiguration$Priority-schema}
    {:gcp.global/name "gcp.bindings.bigquery.QueryJobConfiguration"}))