(ns gcp.bigquery.custom.BigQueryOptions
  ...
  (:require [gcp.bindings.bigquery.DataFormatOptions :as DataFormatOptions]
            [gcp.global :as g])
...
(defn ^BigQueryOptions from-edn
  ([]
   (from-edn nil))
  ([arg]
   (g/strict! :gcp.bigquery.custom/BigQueryOptions arg)
   (if (empty? arg)
...
(defn to-edn
  [^BigQueryOptions arg]
  {:post [(g/strict! :gcp.bigquery.custom/BigQueryOptions %)]}
  (cond-> {}
...
(def schema
  [:or
   {:closed       true,
    ...
    :gcp/key      :gcp.bigquery.custom/BigQueryOptions}
   ...
   [:maybe
    [:map
     [:dataFormatOptions
      ...
      :gcp.bindings.bigquery/DataFormatOptions]
...
(g/include-schema-registry!
  (with-meta {:gcp.bigquery.custom/BigQueryOptions schema}
    {:gcp.global/name "gcp.bindings.bigquery.BigQueryOptions"}))