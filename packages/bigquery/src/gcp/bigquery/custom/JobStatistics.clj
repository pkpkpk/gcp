(ns gcp.bigquery.custom.JobStatistics
  (:require gcp.bindings.bigquery.BiEngineStats
            gcp.bindings.bigquery.DmlStats
            gcp.bindings.bigquery.MetadataCacheStats
            gcp.bindings.bigquery.QueryStage
            gcp.bindings.bigquery.RoutineId
            [gcp.bindings.bigquery.Schema :as Schema]
            gcp.bindings.bigquery.SearchStats
            gcp.bindings.bigquery.TableId
            gcp.bindings.bigquery.TimelineSample
...
(defn base-to-edn [^JobStatistics arg]
  {:post [(global/strict! :gcp.bigquery.custom/JobStatistics %)]}
     ...
     [:stackFrames {:doc "Stack trace showing the line/column/procedure name of each frame on the stack at the point where the current evaluation happened. The leaf frame is first, the primary script is last. Never empty."}
      [:sequential
       [:map {:closed true}
        ...
        [:startLine :int]
        [:text :string]]]]]]
   [:sessionInfo :gcp.bigquery.custom/JobStatistics.SessionInfo]
...
(def JobStatistics$QueryStatistics-schema
  [:map
   [:biEngineStats {:doc "Returns query statistics specific to the use of BI Engine"} :gcp.bindings.bigquery/BiEngineStats]
   [:billingTier {} :int]
   [:cacheHit {} :boolean]
   [:ddlOperationPerformed {} :string]
   [:ddlTargetRoutine {} :gcp.bindings.bigquery/RoutineId]
   [:ddlTargetTable {} :gcp.bindings.bigquery/TableId]
   [:dmlStats {} :gcp.bindings.bigquery/DmlStats]
   ...
   [:exportDataStats
    ...
    [:map {:closed true}
     [:fileCount :int]
     [:rowCount :int]]]
   [:metadataCacheStats {:doc "Statistics for metadata caching in BigLake tables"} :gcp.bindings.bigquery/MetadataCacheStats]
   ...
   [:queryPlan {:doc "Returns the query plan as a list of stages or null if a query plan is not available. Each stage involves a number of steps that read from data sources, perform a series of transformations on the input, and emit an output to a future stage (or the final result). The query plan is available for a completed query job and is retained for 7 days."}
    [:sequential :gcp.bindings.bigquery/QueryStage]]
   [:referencedTables {:doc "Referenced tables for the job. Queries that reference more than 50 tables will not have a complete list"}
    [:sequential :gcp.bindings.bigquery/TableId]]
   ...
   [:timeline {:doc "Return the timeline for the query, as a list of timeline samples. Each sample provides information about the overall progress of the query. Information includes time of the sample, progress reporting on active, completed, and pending units of work, as well as the cumulative estimation of slot-milliseconds consumed by the query."}
    [:sequential :gcp.bindings.bigquery/TimelineSample]]
...
(global/include-schema-registry!
  (with-meta {:gcp.bigquery.custom/JobStatistics.SessionInfo JobStatistics$SessionInfo-schema
              :gcp.bigquery.custom/JobStatistics.QueryStatistics JobStatistics$QueryStatistics-schema
              :gcp.bigquery.custom/JobStatistics schema}
