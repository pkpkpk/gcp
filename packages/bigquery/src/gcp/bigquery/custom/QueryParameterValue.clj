...
(defn ^QueryParameterValue from-edn [arg]
  ...
  (cond
    ...
    (instance? Date arg) (QueryParameterValue/date (.toString arg))
    ...
    (cond
      ...
      (g/valid? [:sequential (g/instance-schema BigDecimal)] arg)
  ...
  (cond
    ...
    (let [raw-value (.getValue arg)]
      (case (.getType arg)
        ...
        StandardSQLTypeName/DATE (LocalDate/parse raw-value)
        StandardSQLTypeName/DATETIME (LocalDateTime/parse raw-value)
        StandardSQLTypeName/TIME (LocalTime/parse raw-value)
        StandardSQLTypeName/TIMESTAMP (Instant/parse raw-value)
