...
(defn ^Publisher from-edn
  [{:keys [batchSettings
           topicName] :as arg}]
  (global/strict! :gcp.pubsub.v1/Publisher arg)
...
(def schemas
  {:gcp.pubsub.v1/Publisher
   [:map {:closed true}
    ...
    [:batchSettings {:optional true} :gcp.pubsub.v1/gax.batching.BatchingSettings]]
   :gcp.pubsub.v1/gax.batching.BatchingSettings
   [:map {:closed true}
    [:delayThreshold {:optional true} :int]
    [:elementCountThreshold {:optional true} :int]
    [:requestByteThreshold {:optional true} :int]
    [:enabled {:optional true} :boolean]
    [:flowControlSettings {:optional true} :gcp.pubsub.v1/gax.batching.FlowControlSettings]]
   :gcp.pubsub.v1/gax.batching.FlowControlSettings :any})
