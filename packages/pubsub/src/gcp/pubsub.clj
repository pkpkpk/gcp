  ...
  ([arg]
   (or *subscription-admin-client*
       (if (instance? SubscriptionAdminClient arg)
         arg
         (g/client :gcp.pubsub.v1/SubscriptionAdminClient arg)))))
  ...
  ([arg]
   (or *topic-admin-client*
       (if (instance? TopicAdminClient arg)
         arg
         (g/client :gcp.pubsub.v1/TopicAdminClient arg)))))
...
(defn list-topics
  ...
  ([arg]
   (if (string? arg)
     ...
     (let [{:keys [topicAdminClient project request]} (g/coerce :gcp.pubsub.v1/synth.TopicList arg)
...
(defn get-topic
  ([arg]
   (cond
     ...
     (g/valid? [:or :gcp.pubsub.v1/TopicName :gcp.pubsub.v1/synth.TopicPath] arg) ;;TODO
     (get-topic {:topic arg})

     true
     (let [{:keys [topicAdminClient topic request]} (g/coerce :gcp.pubsub.v1/synth.TopicGet arg)
           client (topic-admin-client topicAdminClient)]
       (try
         (Topic/to-edn
           (if topic
...
(defn create-topic
  ([arg]
   (if (g/valid? [:or :gcp.pubsub.v1/TopicName :gcp.pubsub.v1/synth.TopicPath] arg)
     (create-topic {:topic arg})
     (let [{:keys [topicAdminClient topic request]} (g/coerce :gcp.pubsub.v1/synth.TopicCreate arg)
           client (topic-admin-client topicAdminClient)]
       (Topic/to-edn
         (if topic
...
(defn list-topic-subscriptions
  ([arg]
   (cond
     ...
     (g/valid? [:or :gcp.pubsub.v1/TopicName :gcp.pubsub.v1/synth.TopicPath] arg)
     (list-topic-subscriptions {:topic arg})

     true
     (let [{:keys [topicAdminClient topic request]} (g/coerce :gcp.pubsub.v1/synth.TopicListSubscriptions arg)
           client (topic-admin-client topicAdminClient)
           response (if topic
...
;; TODO :gcp.pubsub.v1/Topic
(defn delete-topic
  ([arg]
   (if (g/valid? [:or :gcp.pubsub.v1/TopicName :gcp.pubsub.v1/synth.TopicPath] arg)
     (delete-topic {:topic arg})
     (let [{:keys [topicAdminClient topic request]} (g/coerce :gcp.pubsub.v1/synth.TopicDelete arg)
           client (topic-admin-client topicAdminClient)]
       (if topic
...
(defn list-subscriptions
  ([arg]
   (if (string? arg)
     ...
     (let [{:keys [subscriptionAdminClient projectName request]} (g/coerce :gcp.pubsub.v1/synth.SubscriptionList arg)
...
(defn get-subscription
  ([arg]
   (if (string? arg)
     ...
     (if (g/valid? :gcp.pubsub.v1/SubscriptionName arg)
       (get-subscription {:subscription arg})
       (if (g/valid? :gcp.pubsub.v1/Subscription arg)
         (get-subscription {:subscription (:name arg)})
         (let [{:keys [subscriptionAdminClient subscription request]} (g/coerce :gcp.pubsub.v1/synth.SubscriptionGet arg)
               client (subscription-admin-client subscriptionAdminClient)]
           (try
             (Subscription/to-edn
               (if request
...
(defn create-subscription
  ([arg]
   (if (g/valid? :gcp.pubsub.v1/Subscription arg)
     ...
     (let [{:keys [subscription-admin subscription]} (g/coerce :gcp.pubsub.v1/synth.SubscriptionCreate arg)
