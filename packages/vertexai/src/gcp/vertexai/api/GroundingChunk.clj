...
(ns gcp.vertexai.api.GroundingChunk
  {:doc
   ...
   :gcp.dev/certification
     {:base-seed 1774824761178
      :manifest "2e809e6a-933c-51dd-8bb9-567961e7a29e"
      :passed-stages
        {:smoke 1774824761178 :standard 1774824761179 :stress 1774824761180}
      :protocol-hash
        "b44d2581e3979d0a946f2f9d3973f275b05bb339485f3d5eb436c47b54f7dc70"
      :timestamp "2026-03-29T22:52:42.177300175Z"}}
...
(defn ^GroundingChunk$RetrievedContext RetrievedContext-from-edn
  ...
  (let [builder (GroundingChunk$RetrievedContext/newBuilder)]
    ...
    (when (some? (get arg :title)) (.setTitle builder (get arg :title)))
    (when (some? (get arg :uri)) (.setUri builder (get arg :uri)))
    (clojure.core/cond (contains? arg :ragChunk)
                         (.setRagChunk builder
                                       (RagChunk/from-edn (get arg :ragChunk))))
    (.build builder)))
...
(defn RetrievedContext-to-edn
  ...
  (clojure.core/let [res (cond-> {}
                     ...
                     res (case (.name (.getContextDetailsCase arg))
                           "RAG_CHUNK" (clojure.core/assoc res
                                         :ragChunk (RagChunk/to-edn
                                                     (.getRagChunk arg)))
                           res)]
    res))
...
(defn ^GroundingChunk from-edn
  ...
  (let [builder (GroundingChunk/newBuilder)]
    (clojure.core/cond (contains? arg :maps)
                         (.setMaps builder (Maps-from-edn (get arg :maps)))
                       (contains? arg :retrievedContext)
                         (.setRetrievedContext builder
                                               (RetrievedContext-from-edn
                                                 (get arg :retrievedContext)))
                       (contains? arg :web)
                         (.setWeb builder (Web-from-edn (get arg :web))))
    (.build builder)))
...
(defn to-edn
  ...
  (clojure.core/let [res (cond-> {})
                     res (case (.name (.getChunkTypeCase arg))
                           "MAPS" (clojure.core/assoc res
                                    :maps (Maps-to-edn (.getMaps arg)))
                           "RETRIEVED_CONTEXT"
                             (clojure.core/assoc res
                               :retrievedContext (RetrievedContext-to-edn
                                                   (.getRetrievedContext arg)))
                           "WEB" (clojure.core/assoc res
                                   :web (Web-to-edn (.getWeb arg)))
                           res)]
    res))
   ...
   [:map {:closed true}
    [:maps
     {:optional true,
      ...
      :setter-doc
        "<pre>\nGrounding chunk from Google Maps.\n</pre>\n\n<code>.google.cloud.vertexai.v1.GroundingChunk.Maps maps = 3;</code>"}
     :gcp.vertexai.api/GroundingChunk.Maps]
    [:retrievedContext
     {:optional true,
      :getter-doc
      ...
      :setter-doc
        "<pre>\nGrounding chunk from context retrieved by the retrieval tools.\n</pre>\n\n<code>.google.cloud.vertexai.v1.GroundingChunk.RetrievedContext retrieved_context = 2;</code>"}
     :gcp.vertexai.api/GroundingChunk.RetrievedContext]
    [:web
     {:optional true,
      :getter-doc
      ...
      :setter-doc
        "<pre>\nGrounding chunk from the web.\n</pre>\n\n<code>.google.cloud.vertexai.v1.GroundingChunk.Web web = 1;</code>"}
     :gcp.vertexai.api/GroundingChunk.Web]]
   [:or
    [:map
     [:maps
      {:optional true,
