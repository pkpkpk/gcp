...
(ns gcp.vertexai.api.RagRetrievalConfig
  {:doc
   ...
   :gcp.dev/certification
     {:base-seed 1774824589000
      :manifest "2e809e6a-933c-51dd-8bb9-567961e7a29e"
      :passed-stages
        {:smoke 1774824589000 :standard 1774824589001 :stress 1774824589002}
      :protocol-hash
        "b44d2581e3979d0a946f2f9d3973f275b05bb339485f3d5eb436c47b54f7dc70"
      :timestamp "2026-03-29T22:49:49.937062627Z"}}
...
(defn ^RagRetrievalConfig$Filter Filter-from-edn
  ...
  (let [builder (RagRetrievalConfig$Filter/newBuilder)]
    ...
    (clojure.core/cond (contains? arg :vectorDistanceThreshold)
                         (.setVectorDistanceThreshold
                           builder
                           (double (get arg :vectorDistanceThreshold)))
                           ...
                           "VECTOR_DISTANCE_THRESHOLD"
                             (clojure.core/assoc res
                               :vectorDistanceThreshold
                                 (.getVectorDistanceThreshold arg))
                           "VECTOR_SIMILARITY_THRESHOLD"
                             (clojure.core/assoc res
                               :vectorSimilarityThreshold
                                 (.getVectorSimilarityThreshold arg))
                           res)]
    res))
...
(defn ^RagRetrievalConfig$Ranking Ranking-from-edn
  ...
  (let [builder (RagRetrievalConfig$Ranking/newBuilder)]
    (clojure.core/cond (contains? arg :llmRanker) (.setLlmRanker
                                                    builder
                                                    (Ranking$LlmRanker-from-edn
                                                      (get arg :llmRanker)))
                       (contains? arg :rankService)
                         (.setRankService builder
                                          (Ranking$RankService-from-edn
                                            (get arg :rankService))))
    (.build builder)))
...
(defn Ranking-to-edn
  ...
  (clojure.core/let [res (cond-> {})
                     res (case (.name (.getRankingConfigCase arg))
                           "LLM_RANKER" (clojure.core/assoc res
                                          :llmRanker (Ranking$LlmRanker-to-edn
                                                       (.getLlmRanker arg)))
                           "RANK_SERVICE" (clojure.core/assoc res
                                            :rankService
                                              (Ranking$RankService-to-edn
                                                (.getRankService arg)))
                           res)]
  ...
  [:map
   ...
   [:filter
    {:optional true,
     ...
     :setter-doc
       "<pre>\nOptional. Config for filters.\n</pre>\n\n<code>\n.google.cloud.vertexai.v1.RagRetrievalConfig.Filter filter = 3 [(.google.api.field_behavior) = OPTIONAL];\n</code>"}
    :gcp.vertexai.api/RagRetrievalConfig.Filter]
   [:ranking
    {:optional true,
     :getter-doc
     ...
     :setter-doc
       "<pre>\nOptional. Config for ranking and reranking.\n</pre>\n\n<code>\n.google.cloud.vertexai.v1.RagRetrievalConfig.Ranking ranking = 4 [(.google.api.field_behavior) = OPTIONAL];\n</code>"}
    :gcp.vertexai.api/RagRetrievalConfig.Ranking]
   [:topK
    {:optional true,
     :getter-doc
