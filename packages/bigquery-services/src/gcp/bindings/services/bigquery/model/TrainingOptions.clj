...
(ns gcp.bindings.services.bigquery.model.TrainingOptions
  {:doc
   ...
   :gcp.dev/certification
     {:base-seed 1772390265757
      :manifest "2096f8e8-3cdd-50e2-9b64-67d099f5c3be"
      :passed-stages
        {:smoke 1772390265757 :standard 1772390265758 :stress 1772390265759}
      :protocol-hash
        "f22c161c7a00aa071f6a8b78764e5c684eaa491c76675456f17e0b44b2b8578c"
      :timestamp "2026-03-01T18:37:46.526966669Z"}}
  (:require [gcp.bindings.services.bigquery.model.ArimaOrder :as ArimaOrder]
            [gcp.global :as global])
  (:import [com.google.api.services.bigquery.model TrainingOptions]))

(defn ^TrainingOptions from-edn
  [arg]
  (global/strict! :gcp.bindings.services.bigquery.model/TrainingOptions arg)
  (let [o (new TrainingOptions)]
    ...
    (when (some? (get arg :colsampleBylevel))
      (.setColsampleBylevel o (get arg :colsampleBylevel)))
    (when (some? (get arg :colsampleBynode))
      (.setColsampleBynode o (get arg :colsampleBynode)))
    (when (some? (get arg :colsampleBytree))
      (.setColsampleBytree o (get arg :colsampleBytree)))
    ...
    (when (some? (get arg :dataSplitEvalFraction))
      (.setDataSplitEvalFraction o (get arg :dataSplitEvalFraction)))
    ...
    (when (some? (get arg :dropout)) (.setDropout o (get arg :dropout)))
    ...
    (when (some? (get arg :forecastLimitLowerBound))
      (.setForecastLimitLowerBound o (get arg :forecastLimitLowerBound)))
    (when (some? (get arg :forecastLimitUpperBound))
      (.setForecastLimitUpperBound o (get arg :forecastLimitUpperBound)))
    ...
    (when (some? (get arg :horizon)) (.setHorizon o (get arg :horizon)))
    ...
    (when (some? (get arg :initialLearnRate))
      (.setInitialLearnRate o (get arg :initialLearnRate)))
    ...
    (when (some? (get arg :integratedGradientsNumSteps))
      (.setIntegratedGradientsNumSteps o
                                       (get arg :integratedGradientsNumSteps)))
    ...
    (when (some? (get arg :l1RegActivation))
      (.setL1RegActivation o (get arg :l1RegActivation)))
    (when (some? (get arg :l1Regularization))
      (.setL1Regularization o (get arg :l1Regularization)))
    (when (some? (get arg :l2Regularization))
      (.setL2Regularization o (get arg :l2Regularization)))
    ...
    (when (some? (get arg :maxIterations))
      (.setMaxIterations o (get arg :maxIterations)))
    (when (some? (get arg :maxParallelTrials))
      (.setMaxParallelTrials o (get arg :maxParallelTrials)))
    (when (some? (get arg :maxReplicaCount))
      (.setMaxReplicaCount o (get arg :maxReplicaCount)))
    (when (some? (get arg :maxTimeSeriesLength))
      (.setMaxTimeSeriesLength o (get arg :maxTimeSeriesLength)))
    (when (some? (get arg :maxTreeDepth))
      (.setMaxTreeDepth o (get arg :maxTreeDepth)))
    (when (some? (get arg :minAprioriSupport))
      (.setMinAprioriSupport o (get arg :minAprioriSupport)))
    (when (some? (get arg :minRelativeProgress))
      (.setMinRelativeProgress o (get arg :minRelativeProgress)))
    (when (some? (get arg :minReplicaCount))
      (.setMinReplicaCount o (get arg :minReplicaCount)))
    (when (some? (get arg :minSplitLoss))
      (.setMinSplitLoss o (get arg :minSplitLoss)))
    (when (some? (get arg :minTimeSeriesLength))
      (.setMinTimeSeriesLength o (get arg :minTimeSeriesLength)))
    (when (some? (get arg :minTreeChildWeight))
      (.setMinTreeChildWeight o (get arg :minTreeChildWeight)))
    ...
    (when (some? (get arg :numClusters))
      (.setNumClusters o (get arg :numClusters)))
    (when (some? (get arg :numFactors))
      (.setNumFactors o (get arg :numFactors)))
    (when (some? (get arg :numParallelTree))
      (.setNumParallelTree o (get arg :numParallelTree)))
    (when (some? (get arg :numPrincipalComponents))
      (.setNumPrincipalComponents o (get arg :numPrincipalComponents)))
    (when (some? (get arg :numTrials)) (.setNumTrials o (get arg :numTrials)))
    ...
    (when (some? (get arg :pcaExplainedVarianceRatio))
      (.setPcaExplainedVarianceRatio o (get arg :pcaExplainedVarianceRatio)))
    ...
    (when (some? (get arg :sampledShapleyNumPaths))
      (.setSampledShapleyNumPaths o (get arg :sampledShapleyNumPaths)))
    ...
    (when (some? (get arg :subsample)) (.setSubsample o (get arg :subsample)))
    ...
    (when (some? (get arg :timeSeriesLengthFraction))
      (.setTimeSeriesLengthFraction o (get arg :timeSeriesLengthFraction)))
    ...
    (when (some? (get arg :trendSmoothingWindowSize))
      (.setTrendSmoothingWindowSize o (get arg :trendSmoothingWindowSize)))
    ...
    (when (some? (get arg :walsAlpha)) (.setWalsAlpha o (get arg :walsAlpha)))
...
(defn to-edn
  [^TrainingOptions arg]
  {:post [(global/strict! :gcp.bindings.services.bigquery.model/TrainingOptions
                          %)]}
  (cond-> {}
    (.getActivationFn arg) (assoc :activationFn (.getActivationFn arg))
    (.getAdjustStepChanges arg) (assoc :adjustStepChanges
                                  (.getAdjustStepChanges arg))
    (.getApproxGlobalFeatureContrib arg) (assoc :approxGlobalFeatureContrib
    ...
    (.getBoosterType arg) (assoc :boosterType (.getBoosterType arg))
    ...
    (.getCategoryEncodingMethod arg) (assoc :categoryEncodingMethod
                                       (.getCategoryEncodingMethod arg))
    (.getCleanSpikesAndDips arg) (assoc :cleanSpikesAndDips
                                   (.getCleanSpikesAndDips arg))
    (.getColorSpace arg) (assoc :colorSpace (.getColorSpace arg))
    (.getColsampleBylevel arg) (assoc :colsampleBylevel
                                 (.getColsampleBylevel arg))
    (.getColsampleBynode arg) (assoc :colsampleBynode (.getColsampleBynode arg))
    (.getColsampleBytree arg) (assoc :colsampleBytree (.getColsampleBytree arg))
    (.getContributionMetric arg) (assoc :contributionMetric
                                   (.getContributionMetric arg))
    (.getDartNormalizeType arg) (assoc :dartNormalizeType
                                  (.getDartNormalizeType arg))
    (.getDataFrequency arg) (assoc :dataFrequency (.getDataFrequency arg))
    (.getDataSplitColumn arg) (assoc :dataSplitColumn (.getDataSplitColumn arg))
    (.getDataSplitEvalFraction arg) (assoc :dataSplitEvalFraction
                                      (.getDataSplitEvalFraction arg))
    (.getDataSplitMethod arg) (assoc :dataSplitMethod (.getDataSplitMethod arg))
    (.getDecomposeTimeSeries arg) (assoc :decomposeTimeSeries
                                    (.getDecomposeTimeSeries arg))
    (.getDimensionIdColumns arg) (assoc :dimensionIdColumns
                                   (seq (.getDimensionIdColumns arg)))
    (.getDistanceType arg) (assoc :distanceType (.getDistanceType arg))
    ...
    (.getEndpointIdleTtl arg) (assoc :endpointIdleTtl (.getEndpointIdleTtl arg))
    (.getFeedbackType arg) (assoc :feedbackType (.getFeedbackType arg))
    ...
    (.getHiddenUnits arg) (assoc :hiddenUnits (seq (.getHiddenUnits arg)))
    (.getHolidayRegion arg) (assoc :holidayRegion (.getHolidayRegion arg))
    (.getHolidayRegions arg) (assoc :holidayRegions
                               (seq (.getHolidayRegions arg)))
    (.getHorizon arg) (assoc :horizon (.getHorizon arg))
    (.getHparamTuningObjectives arg) (assoc :hparamTuningObjectives
                                       (seq (.getHparamTuningObjectives arg)))
    (.getHuggingFaceModelId arg) (assoc :huggingFaceModelId
                                   (.getHuggingFaceModelId arg))
    ...
    (.getInputLabelColumns arg) (assoc :inputLabelColumns
                                  (seq (.getInputLabelColumns arg)))
    (.getInstanceWeightColumn arg) (assoc :instanceWeightColumn
                                     (.getInstanceWeightColumn arg))
    (.getIntegratedGradientsNumSteps arg)
      (assoc :integratedGradientsNumSteps (.getIntegratedGradientsNumSteps arg))
    (.getItemColumn arg) (assoc :itemColumn (.getItemColumn arg))
    (.getKmeansInitializationColumn arg) (assoc :kmeansInitializationColumn
                                           (.getKmeansInitializationColumn arg))
    (.getKmeansInitializationMethod arg) (assoc :kmeansInitializationMethod
                                           (.getKmeansInitializationMethod arg))
    (.getL1RegActivation arg) (assoc :l1RegActivation (.getL1RegActivation arg))
    ...
    (.getLabelClassWeights arg) (assoc :labelClassWeights
                                  (into {}
                                        (map (fn [[k v]] [(keyword k) v]))
                                        (.getLabelClassWeights arg)))
    (.getLearnRate arg) (assoc :learnRate (.getLearnRate arg))
    (.getLearnRateStrategy arg) (assoc :learnRateStrategy
                                  (.getLearnRateStrategy arg))
    (.getLossType arg) (assoc :lossType (.getLossType arg))
    (.getMachineType arg) (assoc :machineType (.getMachineType arg))
    ...
    (.getMaxReplicaCount arg) (assoc :maxReplicaCount (.getMaxReplicaCount arg))
    ...
    (.getMinReplicaCount arg) (assoc :minReplicaCount (.getMinReplicaCount arg))
    ...
    (.getModelGardenModelName arg) (assoc :modelGardenModelName
                                     (.getModelGardenModelName arg))
    (.getModelRegistry arg) (assoc :modelRegistry (.getModelRegistry arg))
    (.getModelUri arg) (assoc :modelUri (.getModelUri arg))
    (.getNonSeasonalOrder arg) (assoc :nonSeasonalOrder
                                 (ArimaOrder/to-edn (.getNonSeasonalOrder arg)))
    (.getNumClusters arg) (assoc :numClusters (.getNumClusters arg))
    (.getNumFactors arg) (assoc :numFactors (.getNumFactors arg))
    (.getNumParallelTree arg) (assoc :numParallelTree (.getNumParallelTree arg))
    ...
    (.getOptimizationStrategy arg) (assoc :optimizationStrategy
                                     (.getOptimizationStrategy arg))
    (.getOptimizer arg) (assoc :optimizer (.getOptimizer arg))
    (.getPcaExplainedVarianceRatio arg) (assoc :pcaExplainedVarianceRatio
                                          (.getPcaExplainedVarianceRatio arg))
    (.getPcaSolver arg) (assoc :pcaSolver (.getPcaSolver arg))
    (.getReservationAffinityKey arg) (assoc :reservationAffinityKey
                                       (.getReservationAffinityKey arg))
    (.getReservationAffinityType arg) (assoc :reservationAffinityType
                                        (.getReservationAffinityType arg))
    (.getReservationAffinityValues arg) (assoc :reservationAffinityValues
                                          (seq (.getReservationAffinityValues
                                                 arg)))
    ...
    (.getIsTestColumn arg) (assoc :testColumn (.getIsTestColumn arg))
    (.getTfVersion arg) (assoc :tfVersion (.getTfVersion arg))
    (.getTimeSeriesDataColumn arg) (assoc :timeSeriesDataColumn
                                     (.getTimeSeriesDataColumn arg))
    (.getTimeSeriesIdColumn arg) (assoc :timeSeriesIdColumn
                                   (.getTimeSeriesIdColumn arg))
    (.getTimeSeriesIdColumns arg) (assoc :timeSeriesIdColumns
    ...
    (.getTimeSeriesTimestampColumn arg) (assoc :timeSeriesTimestampColumn
                                          (.getTimeSeriesTimestampColumn arg))
    (.getTreeMethod arg) (assoc :treeMethod (.getTreeMethod arg))
    (.getTrendSmoothingWindowSize arg) (assoc :trendSmoothingWindowSize
                                         (.getTrendSmoothingWindowSize arg))
    (.getUserColumn arg) (assoc :userColumn (.getUserColumn arg))
    (.getVertexAiModelVersionAliases arg)
    ...
    (.getXgboostVersion arg) (assoc :xgboostVersion (.getXgboostVersion arg))))
...
(def schema
  [:map
   {:closed true,
    ...
    :gcp/key :gcp.bindings.services.bigquery.model/TrainingOptions}
   [:activationFn
    {:getter-doc
   ...
   [:hiddenUnits
    {:getter-doc
     ...
     :optional true} [:sequential {:min 1} :int]]
   [:holidayRegion
    {:getter-doc
   ...
   [:labelClassWeights
    {:getter-doc
     ...
     :optional true} [:map-of [:or simple-keyword? [:string {:min 1}]] :double]]
   [:learnRate
    {:getter-doc
   ...
   [:nonSeasonalOrder
    {:getter-doc
     ...
     :optional true} :gcp.bindings.services.bigquery.model/ArimaOrder]
   [:numClusters
    {:getter-doc
...
(global/include-schema-registry!
  (with-meta {:gcp.bindings.services.bigquery.model/TrainingOptions schema}
    {:gcp.global/name "gcp.bindings.services.bigquery.model.TrainingOptions"}))