(ns gcp.services.bigquery.custom
...
(defn ^QueryParameterValue QueryParameterValue-from-edn
  [arg]
  (global/strict! ::QueryParameterValue arg)
...
(defn QueryParameterValue-to-edn
  ...
  {:post [(global/strict! ::QueryParameterValue %)]}
  (cond-> {}
  ...
  [:map
   ...
   [:arrayValues
    {:getter-doc
     ...
     :optional true}
    [:sequential {:min 1} [:ref ::QueryParameterValue]]]
   [:rangeValue
    {:getter-doc
     "Optional. The range value, if this is a range type.\n\n@return value or {@code null} for none",
     :setter-doc
     "Optional. The range value, if this is a range type.\n\n@param rangeValue rangeValue or {@code null} for none",
     :optional true} [:ref ::RangeValue]]
   [:structValues
    {:getter-doc
     ...
     :optional true}
    [:map-of [:or simple-keyword? [:string {:min 1}]] [:ref ::QueryParameterValue]]]
   [:value
    {:getter-doc
...
(global/include-schema-registry!
  (with-meta {
              ...
              ::RangeValue RangeValue-schema
              }
             {::global/name "gcp.services.bigquery.custom"}))